...
# Update terakhir: 21 September 2026 (5 artikel terjadwal 22–26 Sep + sistem rilis terjadwal; link Shopee; rapikan SEO)
...
19 artikel, kategori final (JANGAN pakai lagi nama kategori lama "Tips Perawatan/Beli" —
...
15. `backup-data-penting-laptop-hp` — **Perawatan** — tayang 22 Sep 2026 (link WD + Kingston Exodia)
16. `ssd-atau-hdd-laptop-lama` — **Laptop** — tayang 23 Sep 2026
17. `printer-jarang-dipakai-bermasalah` — **Printer** — tayang 24 Sep 2026
18. `storage-laptop-cepat-penuh` — **Laptop** — tayang 25 Sep 2026
19. `ram-8gb-vs-16gb-laptop-kerja` — **Aksesoris** — tayang 26 Sep 2026
...
**RILIS TERJADWAL (21 Sep 2026):** field tanggal boleh ditulis `publishDate` (dipakai artikel baru, sama seperti proyek Ardelia) atau `pubDate` (artikel lama) — keduanya sama. Artikel dengan tanggal di masa depan TIDAK ikut di-build (tidak ada di daftar
Tips, beranda, "Baca juga", sitemap, dan URL-nya belum ada). Logikanya di `src/lib/tips.ts` (`getPublishedTips`), dipakai
oleh `index.astro`, `tips/index.astro`, dan `tips/[...slug].astro`. Tanggal "2026-09-22" berarti tayang 22 Sep 07.00 WIB.
Sama seperti proyek Ardelia Bibit: TIDAK ada pemicu otomatis. Situs statis hanya di-build saat ada push/deploy, jadi
artikel tayang di build PERTAMA setelah tanggalnya lewat (bukan persis pukul 07.00 WIB). Kalau mau tayang tepat harian,
push satu perubahan kecil pada/setelah tanggal itu, atau pasang pemicu otomatis (Vercel Deploy Hook + GitHub Actions
cron) — opsional, belum dipasang. Aturan: (1) isi `pubDate` dengan tanggal rilis yang SEBENARNYA, jangan
di-backdate; (2) artikel tidak boleh menaut ke artikel yang tanggalnya lebih baru (link 404 sampai hari rilis);
(3) uji lokal dengan `PUBLISH_AS_OF=2026-09-24T12:00:00Z npm run build`. 
...
- [x] Link Shopee untuk `tanda-laptop-butuh-servis` (WD My Passport/Elements, WD Elements SE SSD, Kingston Exodia) dan
      ...
      dan menyarankan tes tisu dulu. Artikel backup data sudah ditulis dan dijadwalkan 22 Sep.
